...
export class AppComponent {
  ...
  tabsInfo = [
    { name: 'Friday', map: this.fridayHashMap },
    {
      name: 'Saturday',
      map: this.saturdayHashMap,
    },
    { name: 'Sunday', map: this.sundayHashMap },
  ];
  ...
  serializeData(data: any, hashMap: any) {
    ...
    tempEventList.forEach((event: any, i: number) => {
      ...
      if (!tempEventName.includes('Signing ')) {
        ...
        if (
          tempEventEndTime !== '' &&
          !this.timeValues.some((loc) => loc === tempEventEndTime)
        ) {
          this.timeValues.push(tempEventEndTime);
        }
        const tempEvent = {
          name: tempEventName,
